# Generated by jeweler
# DO NOT EDIT THIS FILE DIRECTLY
# Instead, edit Jeweler::Tasks in Rakefile, and run 'rake gemspec'
# -*- encoding: utf-8 -*-
# stub: gpx2exif 0.3.8 ruby lib

Gem::Specification.new do |s|
  s.name = "gpx2exif"
  s.version = "0.3.8"

  s.required_rubygems_version = Gem::Requirement.new(">= 0") if s.respond_to? :required_rubygems_version=
  s.require_paths = ["lib"]
  s.authors = ["Aleksander Kwiatkowski", "Craig Taverner"]
  s.date = "2014-07-06"
  s.description = "Mass geotagger using GPX files."
  s.email = "bobikx@poczta.fm"
  s.executables = ["geotag", "geotag_all_images", "geotag_simulate", "gpx2png"]
  s.extra_rdoc_files = [
    "LICENSE.txt",
    "README.md"
  ]
  s.files = [
    "Gemfile",
    "Gemfile.lock",
    "LICENSE.txt",
    "README.md",
    "Rakefile",
    "Rakefile'",
    "VERSION",
    "bin/geotag",
    "bin/geotag_all_images",
    "bin/geotag_simulate",
    "bin/gpx2png",
    "lib/geotagger.rb",
    "lib/geotagger/exif_editor.rb",
    "lib/geotagger/geotagger.rb",
    "lib/geotagger/track_importer.rb",
    "lib/gpx2exif.rb",
    "lib/gpx2png/assets/sample_marker.rb",
    "lib/gpx2png/base.rb",
    "lib/gpx2png/osm.rb",
    "lib/gpx2png/osm_base.rb",
    "lib/gpx2png/renderers/chunky_png_renderer.rb",
    "lib/gpx2png/renderers/rmagick_renderer.rb",
    "lib/gpx2png/ump.rb",
    "lib/gpx_utils.rb",
    "lib/mini_exiftool/mini_exiftool.rb",
    "lib/ski_analyzer.rb",
    "lib/ski_analyzer/analyzer.rb"
  ]
  s.homepage = "http://github.com/akwiatkowski/gpx2exif"
  s.licenses = ["LGPLv3"]
  s.rubygems_version = "2.2.2"
  s.summary = "Mass geotagger using GPX files"

  if s.respond_to? :specification_version then
    s.specification_version = 4

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<mini_exiftool>, [">= 0"])
      s.add_runtime_dependency(%q<rmagick>, [">= 0"])
      s.add_runtime_dependency(%q<chunky_png>, [">= 0"])
      s.add_runtime_dependency(%q<gpx_utils>, [">= 0"])
      s.add_runtime_dependency(%q<geokit>, ["= 1.8.4"])
      s.add_development_dependency(%q<rspec>, [">= 0"])
      s.add_development_dependency(%q<bundler>, [">= 0"])
      s.add_development_dependency(%q<jeweler>, [">= 0"])
      s.add_development_dependency(%q<simplecov>, [">= 0"])
    else
      s.add_dependency(%q<mini_exiftool>, [">= 0"])
      s.add_dependency(%q<rmagick>, [">= 0"])
      s.add_dependency(%q<chunky_png>, [">= 0"])
      s.add_dependency(%q<gpx_utils>, [">= 0"])
      s.add_dependency(%q<geokit>, ["= 1.8.4"])
      s.add_dependency(%q<rspec>, [">= 0"])
      s.add_dependency(%q<bundler>, [">= 0"])
      s.add_dependency(%q<jeweler>, [">= 0"])
      s.add_dependency(%q<simplecov>, [">= 0"])
    end
  else
    s.add_dependency(%q<mini_exiftool>, [">= 0"])
    s.add_dependency(%q<rmagick>, [">= 0"])
    s.add_dependency(%q<chunky_png>, [">= 0"])
    s.add_dependency(%q<gpx_utils>, [">= 0"])
    s.add_dependency(%q<geokit>, ["= 1.8.4"])
    s.add_dependency(%q<rspec>, [">= 0"])
    s.add_dependency(%q<bundler>, [">= 0"])
    s.add_dependency(%q<jeweler>, [">= 0"])
    s.add_dependency(%q<simplecov>, [">= 0"])
  end
end

